...
def manu_score(area, hull_area, height,
...
# Processing the input video from webcam.
def process_video(video_path, input_box, min_area=100, output_video="splash_overlay.mp4"):
    import cv2
    import numpy as np
    import pandas as pd
    ...
    for idx in range(frame_count):
        ...
        # Track peak splash. Will be saved later.
    ...
    if best_frame_img is not None and best_frame_mask is not None:
        ...
        cv2.imwrite("best_splash_frame_with_mask.png", overlayed_best)
        cv2.imwrite("best_splash_frame.png", best_frame_img)
    ...
    print(f"  Overlay video saved as {output_video}")
    print("  scores.csv")
